  ...
  .controller('sellerproductaddCtrl', function($scope, $rootScope, $window, $stateParams, $ionicModal, $ionicHistory, $timeout, $ionicPopup, $http, $state, $ionicLoading, $cordovaImagePicker) {
    ...
    if (localStorage.getItem('editId')) {
      ...
      $http.get(baseUrl + 'seller/product/edit/' + data.id, {
        ...
        if ($scope.proEdit.product.media_gallery.images != 0) {
          ...
          for (var i in $scope.proEdit.product.media_gallery.images) {
            ...
            if ($scope.checkimg != $scope.checkimg2) {
              $scope.showaddimage.push({
                "file": $scope.proEdit.product.media_gallery.images[i].file,
                "id": $scope.proEdit.product.media_gallery.images[i].value_id
              })
              console.log($scope.showaddimage)
            } else if ($scope.checkimg == $scope.checkimg2) {
              $scope.showmainimg.push({
                "file": $scope.proEdit.product.media_gallery.images[i].file,
                "id": $scope.proEdit.product.media_gallery.images[i].value_id
              })
            }
        ...
        $http.get(baseUrl + 'seller/subcategory/' + $scope.proEdit.product.category_ids[0], {
          headers: {
            "Authorization": 'Bearer ' + $rootScope.authCode
          }
        }).then(function(res) {
        ...
        $http.get(baseUrl + 'seller/subcategory/' + $scope.proEdit.product.category_ids[1], {
          headers: {
            "Authorization": 'Bearer ' + $rootScope.authCode
          }
        }).then(function(res) {
        ...
        $http.get(baseUrl + 'seller/subcategory/' + $scope.proEdit.product.category_ids[2], {
          headers: {
            "Authorization": 'Bearer ' + $rootScope.authCode
          }
        }).then(function(res) {
    ...
    $http.get(baseUrl + 'seller/subcategory/2', {
      headers: {
        "Authorization": 'Bearer ' + $rootScope.authCode
      }
    }).then(function(res) {
    ...
    $scope.rootcat = function(id) {
      if (id != null || id != undefined || id != '') {
        $rootScope.getCategory = [];
        $http.get(baseUrl + 'seller/subcategory/' + id, {
          headers: {
            "Authorization": 'Bearer ' + $rootScope.authCode
          }
        }).then(function(res) {
      ...
      if ($rootScope.selldata == "Add") {
        ...
        if ($scope.uploadimageAddition.length != 0) {
          for (var i = 0; i < $scope.uploadimageAddition.length; i++) {
            $scope.getalladdition.push({
              "file": $scope.uploadimageAddition[i].format,
        ...
        console.log(data)
        $http.put(baseUrl + 'seller/product/save', data, {
          headers: {
            "Authorization": 'Bearer ' + $rootScope.authCode
          }
                  ...
                  fileEntry.file(
                    function(file) {
                      ...
                      reader.onloadend = function(evt) {
                        ...
                        console.log('test')
                        $scope.uploadimageAddition.push({
                          "file": file.name,
                          "format": $rootScope.imgpickData
                        })
                      };
                      reader.readAsDataURL(file);
    ...
    $scope.imgDelete = function(id) {
      ...
      console.log(data)
      $http.post(baseUrl + 'seller/product/imagedelete', data, {
        headers: {
          "Authorization": 'Bearer ' + $rootScope.authCode
        }
