angular.module('sellerprod', [])
  .controller('sellerproductCtrl', function($scope, $rootScope, $window, $stateParams, $ionicModal, $ionicHistory, $timeout, $ionicPopup, $http, $state, $ionicLoading) {
    ...
    $scope.sellProductInit = function() {
      ...
      $http.get(baseUrl + 'seller/product/' + $rootScope.customerDetails.id, {
        ...
        for (var i in response.data[0].products) {
          $scope.sellerproduct.push({
            "id": response.data[0].products[i].id,
            "name": response.data[0].products[i].name,
            "image": response.data[0].products[i].image,
            "sku": response.data[0].products[i].sku,
            "stock_status": response.data[0].products[i].stock_status,
            "display_status": response.data[0].products[i].display_status,
            "status": false,
            "category": response.data[0].products[i].category,
            "price": response.data[0].products[i].price,
            "created_at": response.data[0].products[i].created_at
          });
    ...
    $scope.productdel = function(id) {
      ...
      $http.post(baseUrl + 'seller/product/delete', {
        product_id: id
      }, {
        headers: {
          "Authorization": 'Bearer ' + $rootScope.authCode
